...
import type { SingleMatchSummary, TrashData, TrashItemKind } from "../services/tournamentApi";
...
export function TrashManager({ tournaments, matches, trash, busy, error, onBack, onTrash, onRestore, onPurge }: { tournaments: ManageTournament[]; matches: SingleMatchSummary[]; trash: TrashData; busy: boolean; error: string; onBack: () => void; onTrash: (kind: TrashItemKind, id: string) => void; onRestore: (kind: TrashItemKind, id: string) => void; onPurge: (kind: TrashItemKind, id: string) => void }) {
  ...
  const currentRows = [
    ...
    ...matches.map((item) => ({ kind: "single" as const, id: item.id, name: `${item.players[0]} v ${item.players[1]}`, meta: `${item.startingScore} · Best of ${item.bestOf}` })),
  ];
  const trashedRows = [
    ...trash.tournaments.map((item) => ({ kind: "tournament" as const, id: item.id, name: item.name, meta: `${item.date} · Tournament` })),
    ...trash.matches.map((item) => ({ kind: "single" as const, id: item.id, name: `${item.players[0]} v ${item.players[1]}`, meta: `${item.startingScore} · Just Play match` })),
  ];
